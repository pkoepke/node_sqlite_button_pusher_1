...
var listenPort = 8080; // was const but that is not supported in strict mode
...
function serveMainPage(request, response) {
  ...
  var responseBody = "";
  // responseBody += fs.readFileSync('material_design_CSS_scripts_links.html')
  ...
  db.all("SELECT ROWID, datetime, clientIp from button_info", function(err,rows) {
    rows.forEach(function(currentRow) {
      dbResponse = "Entry " + currentRow.rowid + ": <span class=\"buttonPushTime\">" + currentRow.datetime + "</span> | Client IP address: " + currentRow.clientIp + "<br />\n";
      responseBody += dbResponse;
    });
    responseBody += "</div>"
